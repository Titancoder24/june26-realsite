    ...
    async (runtimeOptions: Partial<ScribeHookOptions> = {}) => {
      ...
      try {
        ...
        const connectTimeoutId = window.setTimeout(() => {
          rejectConnect?.(new Error("Scribe connection timed out"))
        ...
        abortConnectRef.current = () => {
          settleConnect(new Error("Scribe connection aborted"))
        }
        ...
        connection.on(
          ...
          runIfCurrent((data: unknown) => {
            ...
            setError(message.error)
            setStatus("error")
            settleConnect(new Error(message.error || "Scribe authentication failed"))
            onAuthError?.(message)
          })
        )
        ...
        connection.on(
          ...
          runIfCurrent(() => {
            if (resolveConnect || rejectConnect) {
              settleConnect(new Error("Scribe connection closed unexpectedly"))
            }
            activeConnectionIdRef.current = null
            connectionRef.current = null
            setStatus("disconnected")
